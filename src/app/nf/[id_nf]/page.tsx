...
function InfoRow({ label, value, className = 'text-slate-700 dark:text-zinc-300' }: { label: string; value: React.ReactNode; className?: string }) {
  return (
...
function DataField({ label, value, className = 'text-slate-700 dark:text-zinc-300', large = false }: {
  label: string; value: React.ReactNode; className?: string; large?: boolean;
}) {
  return (
    <div className="space-y-1">
      <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-zinc-500 tracking-wider">{label}</span>
...
function StatCard({ icon: Icon, iconBg, label, value, subValue }: {
  ...
  return (
    <div className="p-4 rounded-xl bg-white/50 dark:bg-zinc-900/50 border border-slate-200 dark:border-zinc-800/50 flex items-center gap-4 shadow-sm backdrop-blur-sm">
      ...
      <div>
        <p className="text-[10px] uppercase font-bold text-slate-500 dark:text-zinc-500 tracking-wider">{label}</p>
...
function StatusBadge({ status }: { status: string }) {
  ...
  let colors = 'bg-slate-100 dark:bg-zinc-500/10 text-slate-600 dark:text-zinc-400 border-slate-200 dark:border-zinc-500/20';
  ...
  if (loading && !nf) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <p className="text-slate-500 dark:text-zinc-500 font-mono animate-pulse">Buscando detalhes da nota fiscal...</p>
      </div>
    );
  }
  ...
  if (notFound || (!loading && !nf && nfs.length > 0)) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center px-4">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Nota Fiscal não encontrada</h2>
        ...
        <button 
          onClick={() => router.push('/')}
          className="px-6 py-3 bg-slate-900 dark:bg-zinc-800 hover:bg-slate-800 dark:hover:bg-zinc-700 text-white rounded-xl transition-colors font-medium flex items-center gap-2 shadow-sm"
        >
          <ArrowLeft size={18} />
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white">NF-e #{nf.numero_nf}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <SectionCard icon={FileText} iconColor="text-blue-500" title="Identificação da NF">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
              ...
              <DataField label="Número NF" value={nf.numero_nf} className="text-slate-900 dark:text-white font-semibold" />
          ...
          <SectionCard icon={Package} iconColor="text-orange-500" title={`Itens da Nota (${detArray.length})`}>
            {detArray.length === 0 ? (
              <p className="text-sm text-slate-500 dark:text-zinc-500 italic">Nenhum item disponível nesta NF</p>
            ) : (
              <div className="overflow-x-auto -mx-6">
                <table className="w-full text-left border-collapse min-w-[700px]">
                  <thead>
                    <tr className="border-b border-slate-200 dark:border-zinc-800/50">
                      <th className="py-3 px-6 text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-wider">#</th>
                      <th className="py-3 px-6 text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-wider">Código</th>
                      <th className="py-3 px-6 text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-wider">Descrição</th>
                      <th className="py-3 px-6 text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-wider">NCM</th>
                      <th className="py-3 px-6 text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-wider">CFOP</th>
                      <th className="py-3 px-6 text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-wider">Unid.</th>
                      <th className="py-3 px-6 text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-wider text-right">Qtd</th>
                      <th className="py-3 px-6 text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-wider text-right">V. Unit.</th>
                      <th className="py-3 px-6 text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-wider text-right">V. Total</th>
                    </tr>
                  </thead>
            ...
            <div className="space-y-4">
              ...
              {(icmsTot.vBC > 0 || icmsTot.vICMS > 0 || icmsTot.vNF > 0) && (
                <div className="p-4 rounded-xl bg-violet-500/5 border border-violet-500/10 space-y-3">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <DataField label="Base de Cálculo" value={fmt(icmsTot.vBC)} className="text-slate-700 dark:text-zinc-300" />
                    <DataField label="Valor ICMS" value={fmt(icmsTot.vICMS)} className="text-violet-500 dark:text-violet-400" />
                    <DataField label="Valor NF" value={fmt(icmsTot.vNF || nf.valor_total_nf)} className="text-slate-900 dark:text-white" />
                    <DataField label="Valor Produtos" value={fmt(icmsTot.vProd)} className="text-slate-700 dark:text-zinc-300" />
                    <DataField label="Valor IPI" value={fmt(icmsTot.vIPI)} className="text-orange-500 dark:text-orange-400" />
                    <DataField label="PIS" value={fmt(icmsTot.vPIS)} className="text-sky-500 dark:text-sky-400" />
                    <DataField label="COFINS" value={fmt(icmsTot.vCOFINS)} className="text-teal-500 dark:text-teal-400" />
                    <DataField label="Valor Frete" value={fmt(icmsTot.vFrete)} className="text-slate-700 dark:text-zinc-300" />
                    <DataField label="Valor Desconto" value={fmt(icmsTot.vDesc)} className="text-amber-500 dark:text-amber-400" />
                    <DataField label="Valor Seguro" value={fmt(icmsTot.vSeg)} className="text-slate-700 dark:text-zinc-300" />
                    <DataField label="Outras Despesas" value={fmt(icmsTot.vOutro)} className="text-slate-700 dark:text-zinc-300" />
                    <DataField label="ICMS Desonerado" value={fmt(icmsTot.vICMSDeson)} className="text-slate-500 dark:text-zinc-400" />
                  </div>
                </div>
              )}
              ...
              {(issqnTot.vISS > 0 || issqnTot.vBC > 0) && (
                <div className="p-4 rounded-xl bg-sky-500/5 border border-sky-500/10 space-y-3">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <DataField label="Base de Cálculo" value={fmt(issqnTot.vBC)} className="text-slate-700 dark:text-zinc-300" />
                    <DataField label="Valor ISS" value={fmt(issqnTot.vISS)} className="text-sky-500 dark:text-sky-400" />
                    <DataField label="Valor Serviços" value={fmt(issqnTot.vServ)} className="text-slate-700 dark:text-zinc-300" />
                  </div>
                </div>
              )}
              ...
              {!icmsTot.vBC && !icmsTot.vICMS && !icmsTot.vNF && !issqnTot.vISS && (
                <p className="text-sm text-slate-500 dark:text-zinc-500 italic">Dados de impostos não disponíveis para esta NF</p>
              )}
            </div>
          </SectionCard>
          ...
          {(compl.xNatureza || compl.cInfCpl || compl.cInfAdFisco) && (
            <SectionCard icon={ClipboardList} iconColor="text-zinc-400" title="Dados Complementares">
              <div className="space-y-4">
                {compl.xNatureza && (
                  <div className="p-4 rounded-xl bg-slate-50/50 dark:bg-zinc-900/50 border border-slate-200 dark:border-zinc-800/50">
                    <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-zinc-500 tracking-wider block mb-2">Natureza da Operação</span>
                    <p className="text-sm text-slate-700 dark:text-zinc-300">{compl.xNatureza}</p>
                  </div>
                )}
                {compl.cInfCpl && (
                  <div className="p-4 rounded-xl bg-slate-50/50 dark:bg-zinc-900/50 border border-slate-200 dark:border-zinc-800/50">
                    <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-zinc-500 tracking-wider block mb-2">Informações Complementares</span>
                    <p className="text-sm text-slate-700 dark:text-zinc-300 whitespace-pre-wrap leading-relaxed">{compl.cInfCpl}</p>
                  </div>
                )}
                {compl.cInfAdFisco && (
                  <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/10">
                    ...
                    <p className="text-sm text-slate-700 dark:text-zinc-300 whitespace-pre-wrap leading-relaxed">{compl.cInfAdFisco}</p>
                  </div>
                )}
              </div>
            </SectionCard>
        ...
        <div className="space-y-6">
          ...
          <SectionCard icon={User} iconColor="text-blue-500" title="Destinatário">
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-slate-50/50 dark:bg-zinc-900/50 border border-slate-200 dark:border-zinc-800/50">
                <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-zinc-500 tracking-wider">Razão Social</span>
                <p className="text-slate-900 dark:text-white font-bold mt-1">{nf.razao_social}</p>
              </div>
              <div className="space-y-0">
                <InfoRow label="CNPJ / CPF" value={nf.cnpj_cpf} className="text-slate-900 dark:text-white font-mono" />
              ...
              {(dest.sEndereco || dest.sCidade || dest.endereco || dest.cidade) && (
                <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/10">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] uppercase font-bold text-blue-500 dark:text-blue-400 tracking-wider">Endereço</span>
                  </div>
          ...
          <SectionCard icon={Info} iconColor="text-cyan-500" title="Informações do Sistema">
            <div className="space-y-0">
              ...
              <InfoRow label="Valor NF" value={fmt(info.nValorNF || nf.valor_total_nf)} className="text-emerald-500 dark:text-emerald-400 font-bold" />
          ...
          {(pedido.nCodPedido || pedido.cNumeroPedido) && (
            <SectionCard icon={ScrollText} iconColor="text-indigo-500" title="Pedido Vinculado">
              <div className="space-y-0">
                ...
                <InfoRow label="Nº Pedido" value={pedido.cNumeroPedido || '--'} className="text-slate-900 dark:text-white font-bold" />
          ...
          {chaveNfe && (
            <SectionCard icon={ShieldCheck} iconColor="text-cyan-500" title="Chave de Acesso NFe">
              <button
                ...
                className="w-full p-3 rounded-xl bg-slate-50 dark:bg-zinc-950/80 border border-slate-200 dark:border-zinc-800/50 text-left flex items-center gap-2 hover:border-cyan-500/30 transition-colors group"
              >
                <code className="text-[10px] text-cyan-600 dark:text-cyan-400/80 font-mono break-all flex-1">
                  {chaveNfe}
                </code>
                {copiedKey ? (
                  <Check size={14} className="text-emerald-500 dark:text-emerald-400 shrink-0" />
                ) : (
                  <Copy size={14} className="text-slate-400 dark:text-zinc-600 group-hover:text-cyan-500 dark:group-hover:text-cyan-400 shrink-0 transition-colors" />
                )}
              </button>
            </SectionCard>
          )}
        ...
        <div className="p-6 rounded-2xl bg-white/50 dark:bg-zinc-900/30 border border-slate-200/50 dark:border-zinc-800/50 backdrop-blur-xl shadow-sm">
          ...
          >
            <h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <Database className="text-slate-400 dark:text-zinc-400" size={20} />
              Dados Brutos (Integração Omie)
            </h2>
            {rawOpen ? (
              <ChevronUp className="text-slate-500 dark:text-zinc-500" size={20} />
            ) : (
              <ChevronDown className="text-slate-500 dark:text-zinc-500" size={20} />
            )}
          </button>
          {rawOpen && (
            <div className="mt-6 bg-slate-50 dark:bg-zinc-950/80 rounded-xl border border-slate-200 dark:border-zinc-800/50 p-4 overflow-x-auto animate-in fade-in slide-in-from-top-2 duration-300">
              <pre className="text-[11px] text-slate-600 dark:text-zinc-400 font-mono leading-relaxed">
