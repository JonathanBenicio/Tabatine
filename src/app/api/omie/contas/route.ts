...
export async function POST(req: Request) {
  try {
    ...
    if (!APP_KEY || !APP_SECRET) {
      return apiError(null, 'POST /api/omie/contas (Missing credentials)', 500);
    }
    ...
    if (axios.isAxiosError(error)) {
      return apiError(error, 'POST /api/omie/contas', error.response?.status || 500);
    }
    return apiError(error, 'POST /api/omie/contas');
  }
}
