...
export default function MeiosTable() {
  ...
  useEffect(() => {
    fetchMeios(currentPage, debouncedSearch);
  }, [fetchMeios, currentPage, debouncedSearch]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Tipos de meios de quitação disponíveis no ERP Omie.</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900/50 border border-slate-700/50 backdrop-blur-xl rounded-2xl overflow-hidden shadow-2xl">
        ...
        {error && (
          <div className="p-6 text-center text-red-400 bg-red-400/10">
            Erro ao carregar meios de pagamento: {error}
          </div>
        )}
        ...
        <div className="overflow-x-auto min-h-[400px]">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-slate-400">
              ...
              <p className="text-lg font-medium">Nenhum meio de pagamento encontrado</p>
            </div>
          ) : (
            <table className="min-w-full divide-y divide-slate-700/50">
              <thead className="bg-slate-800/40">
                <tr>
                  <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wide w-40">
                    Código
                  </th>
