...
const columnHelper = createColumnHelper<MeioPlano>();
...
export default function MeiosTable() {
  ...
  useEffect(() => {
    fetchMeios(currentPage, searchTerm);
  }, [fetchMeios, currentPage, searchTerm]);
    ...
    columnHelper.accessor('codigo', {
      ...
      cell: info => (
        <div className="flex items-center gap-2.5">
          <div className="p-1.5 bg-slate-500/10 dark:bg-zinc-500/10 rounded-lg border border-slate-500/20">
            <Fingerprint size={12} className="text-slate-500 dark:text-zinc-500" />
          </div>
          <span className="font-mono text-[10px] font-bold text-slate-600 dark:text-zinc-400">
            #{info.getValue()}
          </span>
        </div>
      ),
    ...
    <div className="w-full space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-zinc-500 text-sm max-w-md">Tipos de meios de quitação disponíveis no ERP.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <TableSummaryCard
          label="Total de Meios"
          value={totalRegistros}
          sublabel="Tipos cadastrados"
          icon={Coins}
          variant="blue"
          isLoading={loading && meios.length === 0}
        />
        <TableSummaryCard
          label="Sincronizados"
          value={meios.length}
          sublabel="Registros na página"
          icon={Receipt}
          variant="emerald"
          isLoading={loading && meios.length === 0}
        />
      ...
      {error && (
        <div className="p-4 rounded-2xl bg-rose-500/5 border border-rose-500/20 flex gap-4 text-rose-400 animate-in slide-in-from-top-2 duration-300">
          ...
          <div className="text-sm">
            <p className="font-bold tracking-tight">Erro ao carregar meios</p>
