...
export default function FinanceiroTable({ type }: FinanceiroTableProps) {
  ...
  const columns = useMemo(() => [
    columnHelper.accessor('data_vencimento', {
      ...
      cell: info => (
        <div className="flex flex-col">
          <span className="text-sm font-bold text-white tracking-tight">{formatDate(info.getValue())}</span>
          <span className="text-[10px] text-zinc-500 font-mono">Emissão: {formatDate(info.row.original.data_emissao)}</span>
        </div>
      ),
    }),
    columnHelper.accessor('numero_documento', {
      ...
      cell: info => (
        <div className="flex flex-col">
          <span className="text-sm font-bold text-white tracking-tight">#{info.getValue()}</span>
          <span className="text-[10px] text-zinc-500 font-mono">P: {info.row.original.numero_parcela}</span>
        </div>
      ),
    }),
      ...
      cell: info => (
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-zinc-300 truncate max-w-[200px]">{info.getValue()}</span>
          </div>
          <span className="text-[10px] text-zinc-600 font-mono ml-5">{info.row.original.cliente_cnpj_cpf}</span>
        </div>
      ),
    }),
    ...
    columnHelper.accessor('valor_documento', {
      header: 'Valor Bruto',
      cell: info => <span className="text-sm font-medium text-zinc-400">{formatCurrency(info.getValue())}</span>,
      meta: { align: 'right' }
    }),
    columnHelper.accessor('valor_pago_recebido', {
      header: type === 'pagar' ? 'Pago' : 'Recebido',
      cell: info => (
        <span className={`text-sm font-bold ${info.getValue() > 0 ? 'text-emerald-400' : 'text-zinc-500'}`}>
    ...
    columnHelper.accessor('valor_saldo', {
      header: 'Saldo',
      cell: info => (
        <span className={`text-base font-black ${info.getValue() > 0 ? 'text-blue-400' : 'text-zinc-600'}`}>
          {formatCurrency(info.getValue())}
        </span>
      ),
      meta: { align: 'right' }
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-sm">Gerencie seu fluxo de {type === 'pagar' ? 'saídas' : 'entradas'} com precisão.</p>
          </div>
        </div>
